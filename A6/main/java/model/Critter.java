...
public class Critter implements ReadOnlyCritter {
    ...
    public Critter(String s, ProgramImpl p, int[] memory, int c, int r, int direction) {
        ...
        int h1 = Math.abs((s + "a1").hashCode() % 255);
        int h2 = Math.abs((s + "b2").hashCode() % 255);
        int h3 = Math.abs((s + "c3").hashCode() % 255);
        String color = "rgb(" + h1 + "," + h2 + "," + h3 + ")";
        dotColor = Color.web(color, 1);
    ...
    /**
     * @return the Color representing the species of the critter.
     */
    ...
    private int decreaseEnergy(int amt) {
        if (mem[4] - amt <= 0) {
            living = false;
            World.changeContents(this, 0, ((mem[3] * Constants.FOOD_PER_SIZE) + 1) * -1);
            return mem[4];
        }
        mem[4] -= amt;
    ...
    public void forward() {
        ...
        if (living) {
            boolean success = World.moveCritter(this, 1);
            if (success)
                updateCoordinates(1);
        }
    }
    ...
    public void eat() {
        ...
        if (living) {
            ...
            if (remainder > 0)
                World.changeContents(this, 1, (remainder + 1) * -1);
            else
                World.changeContents(this, 1, 0);
        }
    }
    ...
    public void serve(int amt) {
        ...
        if (contents == 0)
            World.changeContents(this, 1, ((amtServed + 1) * -1));
        else {
            World.changeContents(this, 1, contents + (amtServed * -1));
        }
    }
    ...
    public void bud() {
        ...
        if (living) {
            ...
            World.addCritter(c, colBehind, rowBehind);
        }
    ...
    private void mate(Critter partner, Tile tile) {
        ...
        if (living && partner.alive()) {
            ...
            World.addCritter(c, tile.col(), tile.row());
        }
    }
    ...
    public int ahead(int dist) {
        int contents = World.ahead(this, dist);
    ...
    public int nearby(int relDir) {
        int contents = World.nearby(this, relDir);
        int sspp = contents / 10;
        int d = contents % 10;
    ...
    private void flagTile() {
        Tile tile = World.tileAt(col, row);
        World.updatedTiles.add(tile);
    }
